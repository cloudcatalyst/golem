...
Golem is a universal pre-LLM processing layer (spec Decision 22, positioning confirmed by Decision 32): a local-first **TypeScript** service (proxy + MCP server) providing redaction, compression, local tools, routing, and honest observability in front of LLM traffic. Claude Code is Golem's flagship, most-verified integration — byte-faithful proxying, native MCP tools, an agentic developer-assistant layer with local tools (vector knowledge base, local inference over Ollama **or a user-chosen llama.cpp model**, CCR expansion, telemetry) on developer-grade hardware — with the same pipeline designed to extend to other gateways (Foundry, OpenRouter — R6.1/WS-F14, on hold per Decision 36). Compression is *situational* (Decision 23): it pays on non-caching upstreams, ~0% honest savings on Anthropic's cached traffic today. Onboarding/docs live at https://golem.run; npm package **`golem-run`**, CLI binary **`golem`**.

> **The local path is a model fabric, not one tool on one model (spec Decision 56, 2026-08-01).** There is **no default model**: `golem llamacpp models` ranks a curated GGUF ladder against *this* machine's measured free RAM and "no local model is appropriate here" is a valid answer. Two lanes — the **thinker** a conversation plans with and the **coder harness** that drafts and edits — are each pointable at any backend (local llama.cpp, LAN Ollama, an upstream account), and **operating with no upstream at all is a supported, visible end state**. llama.cpp is first-class, not a fallback (`golem llamacpp setup` fetches a pinned upstream build + weights, sha256-verified; Golem ships none of those bytes). Routing per request is gated on **ADR-0005** being accepted.
...
2. **Redeploy locally — LAST, after the final source edit.** This repo dogfoods itself, so a batch is not done until the *running* processes are on the code you just wrote. Run it **after** step 1 is green and after any `biome --write` autofix, and run it **even if you already restarted mid-batch** — a rebuild after that restart leaves a stale daemon, which is the failure mode this step exists to prevent:
   1. `npm run build`
   2. `golem proxy restart` — then **verify**: `golem proxy status` reports a *new* pid, and one command that exercises the change prints the new behaviour (e.g. `golem bench cost`, `golem stats --context`, `golem status`). A restart you did not verify is not a deploy.
   3. Any live `golem mcp serve` connection must be **reconnected by Claude Code** — only needed when an MCP tool's name, schema, or behaviour changed; say so in the summary so the user knows to do it.
   4. `cd vscode-extension && npm run deploy:local` + "Developer: Reload Window" — **only if** extension files changed.
   5. State the outcome in the batch summary: what was rebuilt, the new pid, what you verified, and which of (3)/(4) were not needed and why.

   Docs-only commits after this point need no redeploy; any further source edit means step 2 runs again.
