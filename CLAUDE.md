...
- **A worktree is never disposable because its commit count is zero.** That says nothing about a dirty tree. `git -C <path> status --short` before removing one — uncommitted design work that exists nowhere else looks exactly like an abandoned branch. Commit it on its own branch rather than deciding for the author. **And confirm the owning agent has finished** — not merely that it reported a completed turn, since these agents are resumable; removing the worktree of one that is still verifying destroys the logs it is about to quote (a stash is safe either way: `refs/stash` lives in the common git dir, so `worktree remove` cannot take one) (2026-09-07: cost `team-skills-sync` its verify log, though no committed work).
