...
Golem is an agentic developer-assistant layer for Claude: a local-first **TypeScript** service (proxy + MCP server) that cuts token spend via an embedded lossless compression stage and gives Claude local tools — vector knowledge base, tiered Ollama inference, CCR expansion, telemetry — on developer-grade hardware. Onboarding/docs live at https://golem.run; npm package **`golem-run`**, CLI binary **`golem`**.

> **Naming:** the project was renamed from "EOL / Edge Offload Layer" to **Golem** on 2026-07-03 (spec Decision 19). Older doc sections and dated notes may still say EOL — read them as Golem. Renamed surfaces: `golem_*` MCP tools, `/golem/<cmd>` skills, `/mcp__golem__<cmd>` prompts, `GOLEM_<SECTION>_<KEY>` env, `~/.golem/` + `<project>/.golem/` config, `x-golem-bypass` header.
