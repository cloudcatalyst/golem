...
export function golemGuidanceSection(): string {
  return [
    ...
    "",
    "Check the current savings level (`stats` MCP tool, or `/golem/slider` with",
    "no arguments) — local drafting is available at level 4 (Aggressive) and 5",
    "(Max Savings). At those levels, draft coding changes with the `delegate`",
    "MCP tool first — a local model — and review/refine the result yourself,",
    "rather than writing the draft from scratch. This leaves the paid model's",
    "tokens for the judgment calls the local model can't make: review,",
    "integration, and anything genuinely hard. Skip this below level 4, or when",
    "the task is too small for the round trip to pay off.",
    GUIDANCE_END_MARKER,
  ].join("\n");
}
