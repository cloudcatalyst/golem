...
const SNOOZE_HOLD = [
  ...
  "",
  "Do NOT try to run `golem task add` first. Enforcement denies almost every",
  "non-`snooze` tool call, `Bash` included, so a separate documenting step cannot",
  "run — that is why `note` exists on the tool.",
  "",
  "The exceptions are `ToolSearch` and `expand`, permitted so the park stays",
  "reachable: `snooze` is a deferred tool, and R9.23 found that denying the schema",
  "lookup made the one allowed call impossible to make. Use them for that and",
  "nothing else.",
  "",
  ...
  "every other tool call is denied until you park, so the only way forward is",
