 ...
 * we have NOT already nudged in this session (one-shot). A missing session id
 * still one-shots via a stable fallback key. `sessionKey` is the value the
 * caller must persist so the compared-against and stored keys never diverge.
 */
export function decideCoderFirstNudge(
  target: { isCode: boolean },
  alreadyNudgedSessionId: string | undefined,
  currentSessionId: string | undefined,
): CoderFirstNudgeDecision {
  if (!target.isCode) return { nudge: false };
  const key = currentSessionId ?? NO_SESSION_KEY;
  if (key === alreadyNudgedSessionId) return { nudge: false }; // one-shot per session
...
const stateSchema = z.object({ nudgedSessionId: z.string() });

/** Read the session we last nudged in, or undefined (missing/corrupt → fail-open). */
export async function readCoderFirstNudgeState(projectDir: string): Promise<string | undefined> {
  let raw: string;
  try {
  ...
  try {
    ...
    return parsed.success ? parsed.data.nudgedSessionId : undefined;
  } catch {
    return undefined;
  }
}

/** Record the session we just nudged in (atomic temp+rename in the state dir). */
export async function writeCoderFirstNudgeState(
  ...
  await writeFile(tmp, `${JSON.stringify({ nudgedSessionId: sessionId }, null, 2)}\n`, "utf8");
