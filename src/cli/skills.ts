...
- If the arguments contain a level (0-5), call the \`golem_set_slider\` MCP tool
  with that level, then confirm the change and briefly say what the new level does.
- If no level was given, call \`golem_stats\` and report the current slider level.
...
Call the \`golem_stats\` MCP tool and present the results: current slider level,
...
\`golem_expand\` MCP tool with it. Show the retrieved original content. If the
...
slider level 0 disables all transformation. Call the \`golem_set_slider\` MCP
