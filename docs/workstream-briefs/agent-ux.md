...
## Mission
The developer-facing shell: `npx golem-run init` one-command adoption, the settings
...
- **E1 — Config loader.** `src/config/`: user → project → local → env →
  ...
  Files: `~/.golem/settings.json` (via `env-paths` for the platform-correct base),
  `<project>/.golem/settings.json` (committable), `<project>/.golem/settings.local.json`
  (gitignored). Keys `snake_case`; env overrides `GOLEM_<SECTION>_<KEY>`; zod-validated
  with helpful errors. Typed accessors for other workstreams.
- **E2 — `golem init` / `golem uninit`.** Idempotent, all 3 OSes:
  ...
  3. Register the MCP server: `claude mcp add golem -- golem mcp serve` (or write
     `.mcp.json` at project scope — notes §9).
  4. Install WS-B's skills from `src/mcp/claude-assets/` into
     `.claude/skills/golem/<cmd>/SKILL.md` (→ `/golem/<cmd>`; colon names invalid —
     notes §11) and the PostToolUse hook config (schema in notes §8).
  5. Append guidance to CLAUDE.md via WS-B's writer.
  `golem uninit` reverses every step exactly.
- **E3 — CLI + dashboard v0.** `golem status|slider|stats|index|devices` (commander;
...
- **Consumes:** `src/telemetry/` (read), WS-B's claude-assets + hook CLI, WS-A's
  proxy lifecycle (start/stop/port), `SliderPolicy` for `golem slider`.
...
1. `npx golem-run init` on Win/macOS/Linux configures Claude Code (base URL + MCP +
   skills + hook) idempotently; `golem uninit` cleanly reverses it (DoD #1).
2. T-C2 e2e smoke in `tests/e2e/`: init → Claude Code round-trip with savings > 0
   at level 1 (with WS-A).
3. `golem stats` + dashboard show real per-stage savings attribution (DoD #3).
