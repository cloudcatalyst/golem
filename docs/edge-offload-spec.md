...
25. **ACCEPTED — proxy-side local-drafter intercept, "draft" + "local_first" modes (v1.8, 2026-07-09, USER DECISION).** Extends `golem_delegate` (Decision-adjacent, already built: MCP tool that lets Claude explicitly hand a task to the tiered-Ollama `drafter` role, now `qwen2.5-coder`) with an **automatic, proxy-side trigger** so the local model is engaged without Claude having to ask. This is the combined design discussed when finishing `golem_delegate`: **the delegate call is the engine; the proxy is the policy trigger.** Realizes the §3.3 **Draft/Critic** role that has been on the roadmap since P0 design.
    ...
    - **Mode B — "local_first" (slider level 5 + `slider.local_only_opt_in`, `stages.localOnlyAnswers`).** The pipeline attempts to answer the turn **entirely locally**, skipping the upstream call, when the local draft looks self-contained. **Honest scoping:** `InferenceService.chat()` is text-only by frozen contract (no tool-calling) — it cannot substitute for Claude on any turn that genuinely needs `Read`/`Edit`/`Bash`. The escalation rule is therefore the load-bearing safety rail, not a nice-to-have:
      ...
      4. Otherwise, serve the draft directly: synthesize an Anthropic Messages API response (non-streaming JSON or SSE, matching the shapes in `tests/integration/helpers/anthropic-fixtures.ts` / verification-notes §15, per whether the client requested `stream`) with `stop_reason: "end_turn"`, `model` set to the **real local model id** (never impersonates a Claude model name), and the answer text prefixed *"Answered locally by \<model\> — Golem local-first mode; verify independently."* so the substitution is visible in the Claude Code transcript, not silent.
