...
**Status:** **v1.6 — P0/P1 largely built; roadmap expanded** (see IMPLEMENTATION_PLAN.md, CLAUDE.md, KICKOFF_PROMPT.md)
**Date:** 2026-07-04 (v1.6 adds Decision 22; v1.5 Decision 21; v1.4 Decision 20; v1.3 2026-07-03)
...
21. **Roadmap expansion — multi-model orchestration, remote control & cost governance (v1.5, 2026-07-04, USER DECISION).** Six further directions. Several EXTEND Decision 20 items; where so, noted. None change P0 scope or frozen interfaces; each needs a design memo before build. Two touch serious security surfaces (21b remote permission-granting, 21d/21e credentials) — flagged in the Risks table.
    ...
    - **21b. Remote monitoring, continuation & permission-granting.** Remotely observe session state, actions, and agent updates with a good **step-by-step summary + live conversation view**; **continue the conversation remotely**; and — critically — be **alerted when action is required (especially permission prompts) and grant/deny permissions remotely**. Delivered via a **companion app + a locally-hosted browser dashboard**, with an **optional cloud account**. Extends 20c (remote session access). Hooks into Claude Code's `Notification` / permission events (verification-notes §8). *Phase P3/P4.* **Security-critical:** remote permission approval is remote authorization of code execution — must be strongly authenticated, default-local, auditable; a compromise = RCE. Open: relay/tunnel architecture (self-hosted vs. optional golem.run rendezvous), auth model, offline behavior.
    - **21c. Dashboard-as-sidecar (terminal / VS Code).** Render the Golem dashboard as a **wrapper around the Claude Code conversation** — a sidecar in the terminal or the VS Code UI — surfacing live **token usage, storage, savings, and status** alongside the chat. Extends the E3 dashboard (currently a separate web page) and mirrors what Claude Code's own `/usage`, status line, and VS Code "Account & usage" dialog provide (cost doc, §ref below). *Phase P2.* Open: terminal rendering approach (status line vs. TUI pane) vs. a VS Code webview; how much to lean on Claude Code's status-line hook vs. a standalone panel.
