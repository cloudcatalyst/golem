...
**Status:** **v1.9 — P0/P1 largely built; compression thesis re-based on evidence; local-drafter proxy intercept in build; Golem-managed Ollama bootstrap in build** (see IMPLEMENTATION_PLAN.md, CLAUDE.md, KICKOFF_PROMPT.md)
...
| Local-first mode gives a confident-but-wrong answer to a question that only looks self-contained (Decision 25) | Opt-in only (level 5 + `local_only_opt_in`); any refusal/uncertainty language in the draft escalates to Claude; served-locally answers are visibly labeled in the transcript, never silent; recommend level ≤3 for anything beyond low-stakes/exploratory work |
...
25. **ACCEPTED — proxy-side local-drafter intercept, "draft" + "local_first" modes (v1.8, 2026-07-09, USER DECISION).** Extends `delegate` (Decision-adjacent, already built: MCP tool that lets Claude explicitly hand a task to the tiered-Ollama `drafter` role, now `qwen2.5-coder`) with an **automatic, proxy-side trigger** so the local model is engaged without Claude having to ask. This is the combined design discussed when finishing `delegate`: **the delegate call is the engine; the proxy is the policy trigger.** Realizes the §3.3 **Draft/Critic** role that has been on the roadmap since P0 design.
    ...
    - **Dogfooding:** this repository's own `.golem/settings.json` is configured to slider level 5 + `local_only_opt_in: true` so the maintainer runs local_first day-to-day, per explicit request.
    - **Status:** ACCEPTED, implementing now (not deferred like Decision 24, which this shares its risk profile and response-seam precedent with — Decision 24's "local-answer changes proxy behavior from transport to responder, needs its own contract + recorded-shape tests" applies here verbatim and is satisfied by the tests above). Interfaces touched: `src/proxy/types.ts` (additive field, not `src/interfaces/`), zero changes to `src/interfaces/policy.ts` or `src/interfaces/inference.ts`.
