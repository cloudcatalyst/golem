...
**Date:** 2026-07-09 (v1.9 adds Decision 26; v1.8 Decision 25; v1.7 Decision 23; v1.6 Decision 22; v1.5 Decision 21; v1.4 Decision 20; v1.3 2026-07-03)
...
25. **ACCEPTED — proxy-side local-drafter intercept, "draft" + "local_first" modes (v1.8, 2026-07-09, USER DECISION).** Extends `golem_delegate` (Decision-adjacent, already built: MCP tool that lets Claude explicitly hand a task to the tiered-Ollama `drafter` role, now `qwen2.5-coder`) with an **automatic, proxy-side trigger** so the local model is engaged without Claude having to ask. This is the combined design discussed when finishing `golem_delegate`: **the delegate call is the engine; the proxy is the policy trigger.** Realizes the §3.3 **Draft/Critic** role that has been on the roadmap since P0 design.
    ...
    - **Live slider reload (prerequisite fix, not previously built):** today `golem_set_slider` writes `<project>/.golem/settings.json` but a running `golem proxy` process only reads settings once at startup — the slider has no effect until restart. `buildProxyFromSettings`'s `policy()` now re-reads `slider.level` from the same `SliderStore` per request (cheap local file read; `local_only_opt_in` stays a static project setting, not re-read per request). This makes `golem_set_slider` — the **existing** MCP tool — double as the per-task/per-agent-step toggle: an agent can raise the slider to 4 or 5 before a suitable task and drop it back afterward, with no new MCP tool needed.
