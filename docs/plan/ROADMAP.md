...
_Generated by `golem task index` from `docs/plan/tasks/` — 3 ready, 8 blocked, 83 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R11.7](tasks/R11.7.md) | A request that dies mid-stream leaves no trace — the proxy log has no timestamps and no outcomes | agent | M | — | For any request the proxy handled, its own records can answer WHEN it ran, HOW it ended, and — for a stream that ended without `message_stop` — that it was truncated. A user asking "did Golem do this?" gets an answer from the log rather than an inference. |
