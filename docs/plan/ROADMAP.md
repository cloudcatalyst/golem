...
_Generated by `golem task index` from `docs/plan/tasks/` — 4 ready, 8 blocked, 78 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R11.2](tasks/R11.2.md) | Bound what the session-start index sync may spend — a cap, a checkpoint, and one collection per project | agent | M | — | A session start never begins a multi-minute embed the user did not ask for; an interrupted sync never repeats work it already finished; and one project directory maps to exactly ONE collection however its path is spelled. `golem index` — the explicit ask — is uncapped and unchanged. |
| [R11.3](tasks/R11.3.md) | "`golem off` is in-process only — it silently reverts at the next proxy restart" | agent | S | R11.1 | Turning the pipeline off either PERSISTS — surviving `golem proxy restart`, a SessionStart re-launch and a machine reboot — or says out loud, at the moment it is used, that it lasts only as long as this listener. No third option: a control whose effect disappears without a word is the bug. |
