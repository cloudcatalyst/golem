...
_Generated by `golem task index` from `docs/plan/tasks/` — 3 ready, 17 blocked, 102 done. Edit the task documents, not this table._
...
| [R13.2](tasks/R13.2.md) | "The conversation store — persist redacted transcripts locally, bounded, forgettable, and never in git" | agent | M | — | ADR-0007 is PROPOSED, not accepted — §9 clause 2 (prompt content at rest, versus ephemeral scrollback only) is exactly this task's premise and needs a USER yes first |
| [R13.3](tasks/R13.3.md) | "The session host — Golem spawns and supervises an agent session through its own proxy, and can refuse outright" | agent | L | R13.1, R13.2 | ADR-0007 is PROPOSED, not accepted — §9 clause 1 (remote authorship, and invariant 2's outright refusal) is this task's premise; also needs R13.1's runner verdict |
| [R13.4](tasks/R13.4.md) | "Device pairing for a write surface — LAN-only, minted from Golem's own CA, revocable" | agent | M | — | ADR-0007 is PROPOSED, not accepted — §9 clause 3 (a pairing mechanism that mints device credentials is a new trust root) needs a USER yes first |
| [R13.5](tasks/R13.5.md) | "The transport — stream a hosted session to a device over SSE, take messages by POST, and never show stale state as live" | agent | M | R13.3, R13.4 | ADR-0007 is PROPOSED, not accepted (§9); also needs R13.3's session host and R13.4's pairing, since this is the first write path |
| [R13.6](tasks/R13.6.md) | "The chat surface — a phone-shaped conversation view with a send box, honest about which session it is talking to" | agent | M | R13.5, R12.5 | ADR-0007 is PROPOSED, not accepted (§9); also needs R13.5's transport and R12.5's responsive shell |
| [R13.7](tasks/R13.7.md) | "Join a live session — deliver a device's message as an injected block on the conversation's next request, visibly and only when enforcement is real" | agent | L | R12.12, R13.2, R13.4 | ADR-0007 is PROPOSED, not accepted (§9 clause 1); also gated on R12.12, because without a real `deny` in the receiving session ADR-0007 §3c forbids this path entirely |
| [R13.8](tasks/R13.8.md) | "Start and branch from the device — pick a project, open a hosted session, or fork a recorded conversation from an earlier turn" | agent | M | R13.3, R13.6 | ADR-0007 is PROPOSED, not accepted (§9 clauses 1 and 2 — branching is the feature that requires prompt content at rest); also needs R13.3 and R13.6 |
