...
_Generated by `golem task index` from `docs/plan/tasks/` — 5 ready, 15 blocked, 92 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [ccr-ref-scope](tasks/ccr-ref-scope.md) | "`expand` cannot find a ref issued minutes earlier — the CCR store is per-project-root, and a worktree is a different root" | agent | M | — | A ref a hook issued is retrievable by `expand` for as long as the marker implies, or the marker does not promise it. No path where Golem prints `hash=<64-hex>` and "stored losslessly" over content that cannot be fetched back. |
| [docs-slider-drift-remainder](tasks/docs-slider-drift-remainder.md) | Widen the retired-identifier check to the spec body, the VS Code README and the last stale strings | agent | M | docs-slider-drift | `golem wiki check` scans the spec body and the VS Code README with no issues, and no user-visible string names a control ADR-0004 retired. |
| [R12.11](tasks/R12.11.md) | Spike — Golem as a channel, measured against building the transport ourselves | agent | M | R12.1 | A recommendation with numbers behind it — resume the paused tasks, replace them with a channel, or do neither — plus the one property that decides it. "Neither, and here is what we lose" is a passing answer. |
| [redaction-path-uuid](tasks/redaction-path-uuid.md) | An absolute path containing a UUID is redacted as a secret, so scratchpad and worktree paths cannot be used as command arguments | agent | M | — | A local filesystem path that happens to contain a UUID survives the pipeline intact, and every secret shape the table and the entropy net catch today is still caught. Proven by adding the path case to the existing entropy tests, not by inspection. |
| [subagent-park](tasks/subagent-park.md) | A subagent cannot park — it dies at the usage limit while the parent session is protected | agent | M | — | Work in flight inside a subagent survives a usage limit, or the spawn that would lose it is refused before it starts. Demonstrated against a real limit, or against an injected one — not argued from the code. |
...
| [R12.10](tasks/R12.10.md) | "Capability 3, reopened by the client: decide whether Golem becomes a Claude Code channel, and whether a phone may enqueue a task" | agent | M | R12.5 | needs a USER decision first — whether remote turn-authoring is in scope at all, and the Decision 59(g) amendment §136 asks for |
| [R12.3](tasks/R12.3.md) | "Remote approval as an INPUT to the autonomy gate — a bounded wait, an exact match, and a timeout that denies" | agent | L | R12.1, R12.2 | PAUSED 2026-08-22 (spec Decision 59(i)) pending R12.11 — Anthropic now relays permission prompts to a channel, so this may be redundant as designed. ADR-0006 is accepted; the gate policy is what survives. Original gate: gated on ADR-0006 being accepted (R12.1) — this is the task that turns a phone into an authorizer, and it does not start on a hunch |
| [R12.4](tasks/R12.4.md) | "mTLS device pairing over the LAN — Golem is already a CA, so make it issue client certificates and honour revocation" | agent | L | R12.1 | PAUSED 2026-08-22 (spec Decision 59(i)) pending R12.11 — the mTLS transport exists to carry capability 2, which Anthropic now ships first-party. Original gate: gated on ADR-0006 being accepted (R12.1) |
| [R12.5](tasks/R12.5.md) | "The companion app itself — a phone-shaped view of the locally-hosted dashboard, installable, with approve and deny" | agent | M | R12.1, R12.2, R12.4 | PAUSED 2026-08-22 (spec Decision 59(i)) pending R12.11 — the client waits on which transport it renders over. Original gate: gated on ADR-0006 being accepted (R12.1) |
| [R12.8](tasks/R12.8.md) | The relay — an end-to-end-encrypted rendezvous, both client halves, and a reference server anyone can self-host | agent | L | R12.1, R12.4 | PAUSED 2026-08-22 (spec Decision 59(i)) pending R12.11 — a relay for a capability the platform may already carry. Original gate: gated on ADR-0006 being accepted (R12.1) — and on R12.4, because a relay that carries an mTLS session cannot be built before that session exists |
| [R12.9](tasks/R12.9.md) | Operate a public relay and the accounts that gate it — Golem's first hosted service | user | L | R12.1, R12.8 | PAUSED 2026-08-22 (spec Decision 59(i)) pending R12.11 — do not stand up a hosted service for a capability that may not need one. Still also an outward, credentialed act only the user may take. Original gate: outward infra and credentialed act — DNS, TLS, a server, and a system holding other people's account data. Also gated on R12.8, and on its own identity/PII ADR (see the gate) |
