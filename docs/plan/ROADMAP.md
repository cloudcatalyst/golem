...
_Generated by `golem task index` from `docs/plan/tasks/` — 7 ready, 21 blocked, 119 done. Edit the task documents, not this table._
...
| [settings-cascade-importance](tasks/settings-cascade-importance.md) | "Two bands in the resolver — any origin may declare `!important`, and importance reverses origin order" | agent | M | code | — | "Reversal is asserted per pair, not in aggregate: `user!` beats `team!` beats `project!` beats `local!`, and EVERY important beats EVERY normal. A file origin's `!important` beats `GOLEM_*` — that one reverses a shipped decision, so it gets its own named test. `\"!important\"` naming a key the file does not set WARNS and does not throw. Provenance reports the origin AND `important: true`. A settings file with no `!important` key resolves byte-identically to today — assert against the existing loader fixtures, unchanged." |
...
| [portal-release-webhook](tasks/portal-release-webhook.md) | "The portal end of the release webhook — OIDC on both sides; now needs one repo variable and a release" | user | S | code | release-portal-assets | "CLEARED 2026-09-06 — `golem.run` resolves (`216.198.79.1`), the portal is deployed there, and both repository variables are set (`PORTAL_WEBHOOK_URL`, `PORTAL_OIDC_AUDIENCE`). Nothing is blocked; what remains is the GATE, which only a release from this repo can meet. Details: §156." |
