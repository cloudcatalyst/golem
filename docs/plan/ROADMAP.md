...
_Generated by `golem task index` from `docs/plan/tasks/` — 7 ready, 8 blocked, 34 done. Edit the task documents, not this table._
...
| [R9.5](tasks/R9.5.md) | Managed-file refresh — skills overwrite hand-edits, guidance never updates at all | agent | M | — | A re-init never silently discards a user's edit to a managed file, AND a Golem-side improvement to skill or guidance text actually reaches an already-initialized project. Both halves; either alone is the current bug in mirror image. |
| [R9.6](tasks/R9.6.md) | Config migrations — a renamed setting silently stops working | agent | S | — | A setting renamed or restructured between versions keeps working from an existing settings file, and the user is told once that their file names an old key — rather than the setting silently ceasing to take effect. |
| [R9.7](tasks/R9.7.md) | A Golem-served WebFetch renders as a FAILED tool call | agent | M | — | A cache hit either stops rendering as an error WITHOUT costing an LLM call or a context blow-up, or the cost is measured and the red dot is accepted **in writing** with the reason. Deciding not to change it is a valid outcome; leaving it undecided is not. |
| [R9.8](tasks/R9.8.md) | headroom_config cannot reach nested Headroom config — lossless_only and the CCR markers are unreachable | agent | M | — | A user can turn OFF `<<ccr:…>>` marker emission without dropping the whole slider level — and an unreachable key is REPORTED, never silently dropped. |
| [R9.9](tasks/R9.9.md) | golem config set cannot write an object-valued setting | agent | S | — | `golem config set compression.headroom_config '{"protect_recent":6}'` writes the object — and every `z.record`/object leaf in SETTINGS_LEAVES is settable from the CLI. |
