...
_Generated by `golem task index` from `docs/plan/tasks/` — 6 ready, 9 blocked, 22 done. Edit the task documents, not this table._
...
| [R8.28](tasks/R8.28.md) | mcp/server.ts at 2,209 lines — split into domain modules | agent | L | — | src/mcp/server.ts is under ~700 lines; search/fetch/ingest, wiki_read/wiki_upsert, coder (draft/edit), code+LSP, devices, snooze, and prompts each live in their own module. No behavioral change. |
| [R8.30](tasks/R8.30.md) | Headroom sidecar worker-respawn has no backoff, startup timeout hides stderr | agent | S | — | A dead Headroom worker is not respawned in a tight loop (there is backoff), and a startup failure surfaces the worker's stderr tail so "uv couldn't find Python 3.13" is visible to the user. |
