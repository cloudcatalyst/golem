...
_Generated by `golem task index` from `docs/plan/tasks/` — 7 ready, 8 blocked, 47 done. Edit the task documents, not this table._
...
| [R9.22](tasks/R9.22.md) | `golem init` writes a machine-absolute CA path into the COMMITTED .claude/settings.json | agent | S | — | A fresh clone on a different machine starts Claude Code with no certificate warnings, and a developer who runs `golem init` does not produce a diff that would break a teammate. Proven by checking out the repo at a different path and starting a session. |
