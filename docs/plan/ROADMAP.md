...
_Generated by `golem task index` from `docs/plan/tasks/` — 8 ready, 8 blocked, 66 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R10.12](tasks/R10.12.md) | Decision 56's bypass shim is gone — `proxy stop` again leaves Claude Code wired to a dead port | agent | M | — | After `golem proxy stop`, a Claude Code request still completes (served by the redaction-only shim) OR the project is honestly unwired with the required reload surfaced — never a silent connection-refused. `status --json` emits `proxy.bypass` while the shim is serving, and the VS Code panel/status bar say "bypass — pipeline off" rather than "running". |
| [R10.13](tasks/R10.13.md) | `golem status` reports a rebuilt-but-not-restarted daemon as current — staleness is version-only | agent | S | — | After `npm run build` with no version change, `golem status` reports the running daemon as stale and names the fix. A restart clears it. A daemon that IS the current build is never reported stale (no false positive on the common case). |
| [R10.16](tasks/R10.16.md) | Translated SSE sends nothing until the first token — no eager `message_start`, no `ping` | agent | M | — | A translated streaming response emits `message_start` before the upstream's first token and keeps the stream alive during a long prefill. A client watching a slow request sees activity throughout, not a dead socket followed by a burst. |
| [R10.17](tasks/R10.17.md) | The CCR oversized-output swap has never once fired for `Read` — its text is nested at `file.content` | agent | S | — | A `Read` of a file larger than the inline threshold is swapped for a head/tail excerpt plus a `hash=` ref, proven on a real transcript shape. Every `tool_response` shape Golem does not understand still passes through untouched. |
| [R10.18](tasks/R10.18.md) | An empty upstream completion is manufactured into a valid empty answer — surface it as an error | agent | S | — | When an OpenAI-schema upstream returns no content at all, the client receives an error saying so — naming the model and that it produced nothing — instead of a well-formed response containing "". A response with real content is untouched on both paths. |
| [R10.19](tasks/R10.19.md) | headroom_config keys are validated only when Headroom runs — on a caching upstream they are silently dropped after all | agent | S | — | A `compression.headroom_config` key that cannot reach Headroom is reported on a surface the user reads, INCLUDING when the semantic stage never runs. Proven on an OpenRouter target, where the stage is always off. |
