...
_Generated by `golem task index` from `docs/plan/tasks/` — 6 ready, 8 blocked, 36 done. Edit the task documents, not this table._
...
| [R9.10](tasks/R9.10.md) | the local-coder naming is wrong — workers route to any target, and every surface still says local | agent | M | — | No surface claims a worker is "local" when its target is not. `golem status --json` never reports a non-local model under a `local_model` key, and the enable/disable switch for the `coder` tool is named for the tool, not for where it runs. |
