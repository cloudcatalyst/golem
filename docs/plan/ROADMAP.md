...
| R7.6 | ~~Interactive control panel + one config surface: `src/config/ui-model.ts` (compile-exhaustive setting metadata, zod-derived widget kinds) + `src/config/control-surface.ts` (settings + guidance + runtime as one `Control` list, env-locked rows, danger confirms) behind `golem ui` / bare `golem` (ink, pure reducer, lazy-imported), `golem config schema [--json]`, and a Settings section in the VS Code webview.~~ — **DONE** (Dec 50): the config loader was untouched; only presentation was missing. **Caveat:** `golem ui` inside the Bun standalone binaries is UNVERIFIED (yoga-layout WASM — folds into R7.3's 🔬 smoke-test, verification-notes §85). | ✅ | Dec 50 |
...
2. **🔬 R7.3 standalone-binary verification** — the Bun `--compile` binaries are
   ...
   billing-blocked). Smoke-test each before relying on the binary channel. **Now
   also covers `golem ui`**: ink depends on `yoga-layout` (WASM), and whether
   `bun build --compile` embeds that asset is unverified (verification-notes §85).
   If it doesn't, `runTui` must fail with "use the npm install for the panel"
   rather than crash.
