...
_Generated by `golem task index` from `docs/plan/tasks/` — 3 ready, 15 blocked, 87 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [docs-slider-drift](tasks/docs-slider-drift.md) | The README still documents the slider R11.1 retired, and shows version 0.1.1 | agent | S | — | No user-facing document describes a control that no longer exists, no version string is hand-written where it will go stale, and a check FAILS on the next such drift — so this cannot reopen quietly the way R11.5's wiki Index did. |
| [R12.1](tasks/R12.1.md) | "ADR-0006 — the threat model for remote steering: write it, then decide whether the companion app gets built" | agent | M | — | "An ADR the user has ACCEPTED, before any of R12.3–R12.7 starts. It must answer, in the user's own words: what a paired phone can authorize, what it can never authorize, what happens when the link drops, and what a stolen phone gets the attacker. Where a mitigation does not exist, the ADR says so plainly rather than implying one — the ADR-0005 precedent." |
| [R12.2](tasks/R12.2.md) | "The blocked-state file says the session is stuck but not on what — widen it into a documented read model, with no network surface" | agent | M | — | "One documented read model that answers WHICH project, WHICH session, blocked on WHAT, and SINCE WHEN — redacted before it is written — and that the status line, the VS Code panel and the dashboard all render from. No listener binds anywhere new, and nothing gains write access. Verified by a live block: trigger a real permission prompt and read the file." |
...
| [R12.3](tasks/R12.3.md) | "Remote approval as an INPUT to the autonomy gate — a bounded wait, an exact match, and a timeout that denies" | agent | L | R12.1, R12.2 | gated on ADR-0006 being accepted (R12.1) — this is the task that turns a phone into an authorizer, and it does not start on a hunch |
| [R12.4](tasks/R12.4.md) | "mTLS device pairing over the LAN — Golem is already a CA, so make it issue client certificates and honour revocation" | agent | L | R12.1 | gated on ADR-0006 being accepted (R12.1) |
| [R12.5](tasks/R12.5.md) | "The companion app itself — a phone-shaped view of the locally-hosted dashboard, installable, with approve and deny" | agent | M | R12.1, R12.2, R12.4 | gated on ADR-0006 being accepted (R12.1) |
| [R12.6](tasks/R12.6.md) | "Spike — how does the phone learn the agent is blocked, without putting a third party in the path? (expect a constrained answer)" | agent | S | R12.1 | gated on ADR-0006 being accepted (R12.1) |
| [R12.7](tasks/R12.7.md) | Continue is not approve — decide what a phone can honestly resume, given that no reverse channel into the TUI exists | agent | S | R12.1 | gated on ADR-0006 being accepted (R12.1) |
| [R12.8](tasks/R12.8.md) | The relay — an end-to-end-encrypted rendezvous, both client halves, and a reference server anyone can self-host | agent | L | R12.1, R12.4 | gated on ADR-0006 being accepted (R12.1) — and on R12.4, because a relay that carries an mTLS session cannot be built before that session exists |
| [R12.9](tasks/R12.9.md) | Operate a public relay and the accounts that gate it — Golem's first hosted service | user | L | R12.1, R12.8 | outward infra and credentialed act — DNS, TLS, a server, and a system holding other people's account data. Also gated on R12.8, and on its own identity/PII ADR (see the gate) |
