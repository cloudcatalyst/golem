...
_Generated by `golem task index` from `docs/plan/tasks/` — 10 ready, 8 blocked, 49 done. Edit the task documents, not this table._
...
| [R10.3](tasks/R10.3.md) | Headroom sidecars outlive the proxy that spawned them — 24 orphaned Python processes over five days | agent | M | — | Stopping or restarting the proxy leaves no `uv`/python child behind — verified by counting python processes before and after a stop/start cycle on Windows, where the leak reproduces. No orphan may survive its parent daemon. |
| [R10.4](tasks/R10.4.md) | A hardware-tier downgrade silently swaps the embedder, and every local-answer query then fails on vector width | agent | M | — | A project whose index was built with one embedder never attempts a query with a different-width one. Either the query uses the embedder the index was built with, or local-answer is disabled with a message naming both models — never an error per query. |
