...
_Generated by `golem task index` from `docs/plan/tasks/` — 9 ready, 8 blocked, 54 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R10.7](tasks/R10.7.md) | Tier P_MIN pairs a 1024-dim text embedder with a 768-dim code embedder in ONE collection | agent | S | — | A single collection can never be asked to hold two vector widths. Either the tier pairs same-width models, or the driver keeps text and code vectors apart — proven by ingesting both a text and a code file on P_MIN without a reset. |
| [R10.8](tasks/R10.8.md) | `coder` falls through to the LOCAL model when no target is named — it should fall through to inference.default_target, then the harness default | agent | M | — | With no `worker_targets` entry and no explicit target, `coder` dispatches to `inference.default_target`; with that unset it uses the harness's own default upstream. A local backend is reachable ONLY by naming a target that points at it. `golem status` reports which of the three the next dispatch would use. |
