...
_Generated by `golem task index` from `docs/plan/tasks/` — 9 ready, 8 blocked, 56 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R10.10](tasks/R10.10.md) | The VS Code status bar flaps online/offline — four full CLI startups per poll against an 8s timeout | agent | M | — | The status bar does not flip to offline while the proxy is up, under a machine load that currently reproduces it. Proven by leaving the extension polling during a full `vitest run` and watching it stay green. |
| [R10.9](tasks/R10.9.md) | A loopback `local`-trust target is dispatched to the Ollama service, whatever it actually points at | agent | M | R10.8 | Declaring a loopback `llamacpp` target and routing `coder` at it reaches THAT llama.cpp server — provable from the audit record and the served model — with no Ollama installed at all. No new unredacted egress path is introduced. |
