...
_Generated by `golem task index` from `docs/plan/tasks/` — 7 ready, 8 blocked, 68 done. Edit the task documents, not this table._
...
| [R10.20](tasks/R10.20.md) | Thinking blocks are synthesized on the way out and discarded on the way back — the round trip is asymmetric | agent | M | — | A decision is recorded and implemented for what a reasoning model receives of its OWN prior reasoning on an OpenAI-schema upstream. Whatever is chosen, the two directions agree, and a dropped block is dropped deliberately rather than by omission. |
