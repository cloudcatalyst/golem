...
| [portal-release-webhook](tasks/portal-release-webhook.md) | "The portal end of the release webhook — OIDC on both sides; now needs one repo variable and a release" | user | S | code | release-portal-assets | "**`golem.run` has no A record** (observed 2026-09-05: `curl: (6) Could not resolve host`). The code is done on both sides and the loop was RUN end to end against a local portal behind a tunnel — `HTTP 200 {\"version\":\"0.52.1\",\"stored\":true}`. What remains is DNS + a portal deploy, then set `vars.PORTAL_WEBHOOK_URL` (left unset on purpose: it is what keeps the job inert instead of red every release)." |
