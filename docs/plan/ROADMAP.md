...
| [R9.7](tasks/R9.7.md) | A Golem-served WebFetch renders as a FAILED tool call | agent | S | — | A cache hit either stops rendering as an error, or the cost of making it render green is measured and the red dot is accepted **in writing** with the reason. Deciding not to change it is a valid outcome; leaving it undecided is not. |
