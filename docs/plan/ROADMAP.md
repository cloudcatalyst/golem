...
_Generated by `golem task index` from `docs/plan/tasks/` — 9 ready, 8 blocked, 57 done. Edit the task documents, not this table._

### Ready to pick up

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [R10.11](tasks/R10.11.md) | `vscode-extension/render.test.js` is run by nothing, and had rotted for four releases | agent | S | — | The extension's tests run in the same command as everything else and are green. A field the CLI renames cannot reach a release with the extension still reading the old name. |
