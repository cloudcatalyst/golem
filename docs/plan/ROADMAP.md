...
_Generated by `golem task index` from `docs/plan/tasks/` — 5 ready, 9 blocked, 29 done. Edit the task documents, not this table._
...
| [R9.2](tasks/R9.2.md) | Proxy serves many targets — pool registry + virtual model ids | agent | M | R9.1 | A sub-agent defined with `model: golem/<id>` is served by that target, its real serving model is visible in `golem status`, and every existing byte-fidelity / recorded-shape test passes unmodified. |
| [R9.3](tasks/R9.3.md) | coder runs on any target — with redaction before every non-local dispatch | agent | M | R9.1 | `coder` completes a draft on a remote target, AND a recorded test proves the dispatched payload carries no secret/PII for any non-local target while the returned draft is fully de-redacted. |

### Blocked or waiting (visible, not lost)

| task | goal | owner | size | depends on | gate / blocker |
|---|---|---|---|---|---|
| [21e](tasks/21e.md) | Per-request capability/availability routing — multi-target proxy | user | L | R9.1, R9.2, R9.3 | umbrella only — nothing to implement here; it closes when R9.1, R9.2 and R9.3 land. Agents should pick up those three, not this. |
