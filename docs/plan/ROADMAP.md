...
| R7.7 | ~~CLI startup latency: `main.ts` as a dependency-free router, `fast-path.ts` for the per-tool-call hooks + status line, read/write module splits (`slider-read.ts`, `upstream-display.ts`), barrel-import narrowing (`undici` off the hot paths), and **ink/React removed** — `src/tui/` renders itself via `render/screen/keys/ansi/width`.~~ — **DONE** (Dec 51): panel first frame ~2.5–3s → **~170ms**; `hook post-tool-use` ~765ms → **126ms**; `statusline` ~985ms → **275ms**; runtime deps back to **6**. The proxy was measured at **+4.4ms p50** per request and needed no work. Notes §86/§86b/§86c. | ✅ | Dec 51 |
