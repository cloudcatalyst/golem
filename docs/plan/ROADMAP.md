...
_Generated by `golem task index` from `docs/plan/tasks/` — 7 ready, 21 blocked, 118 done. Edit the task documents, not this table._
...
| [release-pr-needs-approval](tasks/release-pr-needs-approval.md) | "The release PR cannot go green on its own — a bot-opened PR does not trigger CI, so every release stalls at `action_required`" | agent | S | code | — | "Cutting a release requires no manual approval step — or, if the manual beat is kept deliberately, `release-prepare.yml` and the Release Pipeline page SAY so, and the PR body tells the human what they must click." |
...
| [skills-project-scope-reachability](tasks/skills-project-scope-reachability.md) | "Project-scoped `/golem:*` skills may be unreachable — and a user-scope plugin is currently masking it on one machine" | agent | M | code | — | "On a machine with NO `~/.claude/skills/golem/`, a freshly `golem init`-ed project lists every `/golem:<cmd>` skill. Proven by a real listing, not by reading the installer." |
...
| task | goal | owner | size | discipline | depends on | gate / blocker |
|---|---|---|---|---|---|---|
| [npm-token-set-but-broken](tasks/npm-token-set-but-broken.md) | "`NPM_TOKEN` is set but does not work — so every release attempts a publish and goes red after succeeding" | user | S | code | — | "changing or removing a repository secret is a credentialed act only the account owner can take" |
...
| [R13.17](tasks/R13.17.md) | Local `npm test` takes many minutes and pins every core — cut the wall-time floor and stop running the full suite per batch | agent | M | — | — | HALF SHIPPED. The remaining half — splitting `cli-init.test.ts` and `cli-status.test.ts` — cannot start until `R13.16-skills-plugin-user-scope` merges to `main`; that branch rewrites both files (+319/+82 lines) and a split-vs-rewrite is not a resolvable merge. |
