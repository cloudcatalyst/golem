...
_Generated by `golem task index` from `docs/plan/tasks/` — 10 ready, 8 blocked, 51 done. Edit the task documents, not this table._
...
| [R10.5](tasks/R10.5.md) | The MEMORY sidecar is never copied to dist/, so it cannot launch from a built install | agent | S | — | A built install (`npm run build`) can launch the MEMORY sidecar. Proven by running the built CLI with `knowledge.memory_federation_enabled` on and seeing the worker come up, not fail open. |
| [R10.6](tasks/R10.6.md) | The knowledge BUILD side still picks its embedder by detected tier | agent | S | R10.4 | A transient capability-probe failure cannot silently re-embed an index at a different width. Either the build refuses and says why, or it records what it did loudly enough that the next query is not surprised. |
