...
| [portal-release-webhook](tasks/portal-release-webhook.md) | "The portal end of the release webhook — OIDC on both sides; now needs one repo variable and a release" | user | S | code | release-portal-assets | "the CODE is done on both sides. What remains is one repository VARIABLE: `vars.PORTAL_WEBHOOK_URL` = the portal's `/api/webhooks/golem-build`. No shared secret exists any more — OIDC removed it. Then cut a release (or dispatch Release with `notify_only`)." |
