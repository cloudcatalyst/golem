...
- debriefs/2026-07-23-statusline-golem-dir-gating.md — status surfaces run in every project, so they must not footprint non-Golem repos: best-effort writers never bootstrap `.golem/` (PR #20), the VS Code bar hides itself outside Golem projects, and the "off" state (proxy stopped or level 0) now reads `Passthrough → [local + ]<upstream>` on both surfaces (PR #25)
