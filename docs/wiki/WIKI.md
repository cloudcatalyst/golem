...
- debriefs/2026-07-23-webfetch-raw-cache.md — spec Decision 42: WebFetch caches the RAW page (Golem fetches it itself in PostToolUse via `fetchRawPage`), not Claude Code's prompt-specific answer; a failed fetch caches nothing, legacy answer-entries self-heal on re-fetch; config `knowledge.webcache_fetch_raw` (default on); the local-answer length-gate stays (proxy path, not superseded)
