...
- [[Local Model Fabric]] — Decision 56: **no default model**; why a 20 GB MoE beats an 8 GB dense model on 8 GB of VRAM (active parameters, not file size), `golem llamacpp` install/run/verify, and the platform facts that are data rather than patterns (§114)
...
- concepts/Plugin Seams.md — R8.11/ADR-0004: how a third party extends Golem without forking — the three seams and their three trust classes, what holds by construction (append-only, no impersonation, byte-fidelity untouchable, determinism), and the two things Golem deliberately does NOT claim (no sandbox for `stage`/`tool`; ReDoS is a documented residual risk)
- debriefs/2026-08-01-r8.11-plugin-seams.md — R8.11: the plugin surface, and the finding that inverted the design — the seam with the most demand (org-private redaction rules) is also the one pointed at unredacted secrets, but everything it needs to express is DATA, so it accepts a pattern and a validator NAME and runs no third-party code at all; stage/tool run unsandboxed and are placed where that survives (post-redaction lossy slot; off the request path). Append-only enforced by the absence of a mechanism, asserted against the module's exported names. Two bugs in the over-redaction cap, both from measuring length instead of matched characters: it misfired on ordinary use (placeholders are longer than secrets) AND missed the total swallow it existed for. ReDoS lint-and-measure only; the out-of-process host was declined on purpose
- debriefs/2026-08-02-r8.18-llamacpp.md — R8.18: `golem llamacpp models|setup|status|start|stop`, the first-class llama.cpp backend; no default model (ranked against measured free RAM with "no local model is appropriate" as an answer), resumable + sha256-verified downloads against publisher-reported digests, per-release install dirs, detached server with a log. Four platform bugs found by running it live: no Linux CUDA asset (→Vulkan), bsdtar reading `C:\` as a host, the 3-second probe runner killed an extraction, and `--model-draft` without `--spec-type draft-mtp` segfaulting the server; a 15.7 tok/s end-to-end measurement on a 35B MoE over 8 GB VRAM. §114, §115.
