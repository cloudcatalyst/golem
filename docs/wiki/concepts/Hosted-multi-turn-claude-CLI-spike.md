...
| item | label | result |
|---|---|---|
| Multi-turn input | **[OBSERVED]** | -p --input-format stream-json --output-format stream-json accepts second message after first esult event, same session_id kept |
| Structured output | **[OBSERVED]** | Tool calls and tool results arrive as separate event objects correlatable by 	ool_use_id |
...
| Permission behaviour | **[OBSERVED]** | Headless sk resolves to synchronous refusal — no dialog possible |
| Proxy interposition | **[OBSERVED]** | Traffic flows wherever ANTHROPIC_BASE_URL points |
| Cost/lifetime | **[OBSERVED]** rough | .21-.51 per trivial turn cold-cache at Opus-5; idle costs nothing beyond OS process |
