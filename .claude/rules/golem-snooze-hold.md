...
Do NOT try `golem task add` first — enforcement denies every non-`snooze` tool call, which is why `note` exists on the tool.

By default this is **enforcing** (Decision 45): every non-`snooze` call denied until you park. Set `snooze.enforce` false or `GOLEM_SNOOZE_ENFORCE=false` for advisory (one nudge per window). `golem status` shows `park advisory | enforced`.
